...
st = time.clock();
append='/home/hltcoe/gsell/code/recommend/yelp/'
test_cond='1_5';

print 'Loading lists...'
business_lst = [l for l in open('%sprojects/csv/business_vertices_%s.csv'%(append,test_cond)).read().split('\n') if len(l)>0];
all_reviewer_lst = [l for l in open('%sprojects/csv/reviewer_vertices_%s.csv'%(append,test_cond)).read().split('\n') if len(l)>0];
...
train_edges = [x.split(',') for x in open('%sprojects/csv/bipartite_train_edges_%s.csv'%(append,test_cond)).read().split('\n') if len(x)>0 and 'reviewer' not in x];
...
seed_edges = [x.split(',') for x in open('%sprojects/csv/bipartite_seed_edges_%s.csv'%(append,test_cond)).read().split('\n') if len(x)>0 and 'reviewer' not in x];
...
test_edges = [x.split(',') for x in open('%sprojects/csv/bipartite_test_edges_%s.csv'%(append,test_cond)).read().split('\n') if len(x)>0 and 'reviewer' not in x];
